...
const inputSchema = z.object({
    ...
    from: z
        .union([z.tuple([z.number().int(), z.number().int(), z.number().int()]), z.tuple([z.number().int(), z.number().int()])])
        .describe(
            "Inclusive corner of the region: [x, y, z], or just [x, z] for view \"top\"/\"heightmap\" (the surface is then found " +
                "automatically over the whole world height). Give a y range to a top view only when you want to look under a roof or underground."
        ),
    to: z
        .union([z.tuple([z.number().int(), z.number().int(), z.number().int()]), z.tuple([z.number().int(), z.number().int()])])
        .describe("Inclusive corner opposite `from`, same shape as `from`. Order does not matter."),
    view: z
        .enum(VIEWS)
        .optional()
        .describe(
            '"top" (default): top-down layout, shaded like a vanilla in-game map. "north"/"south"/"east"/"west": ' +
                ...
                "an x/z area only, area-priced like mc_survey rather than volume-priced."
        ),
    slice: z
        .object({
    ...
    server.registerTool(
        ...
        async ({ world, from, to, view, slice, scale, grid, heightmapType, contour }) => {
            return runToolContent(client, "mc_render", async () => {
                ...
                const footprintOnly = from.length === 2 || to.length === 2;
                if (footprintOnly && resolvedView !== "top" && resolvedView !== "heightmap") {
                    throw new Error(`mc_render view "${resolvedView}" needs [x, y, z] corners; only "top" and "heightmap" accept [x, z].`);
                }
                if (footprintOnly && from.length !== to.length) {
                    throw new Error("mc_render: from and to must both be [x, z] or both be [x, y, z].");
                }
                // Normalize to x/z (+ optional y) so the checks below work for both shapes.
                const fx = from[0], fz = from.length === 2 ? from[1] : from[2];
                const tx = to[0], tz = to.length === 2 ? to[1] : to[2];
                const fy = from.length === 3 ? from[1] : 0, ty = to.length === 3 ? to[1] : 0;
                const [x1, y1, z1] = [Math.min(fx, tx), Math.min(fy, ty), Math.min(fz, tz)];
                const [x2, y2, z2] = [Math.max(fx, tx), Math.max(fy, ty), Math.max(fz, tz)];
                ...
                const result = (await client.request("render", {
                    world,
                    from: resolvedView === "heightmap" || footprintOnly ? [x1, z1] : [x1, y1, z1],
                    to: resolvedView === "heightmap" || footprintOnly ? [x2, z2] : [x2, y2, z2],
                    view,
                    slice,
                    scale,
                    grid,
